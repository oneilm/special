...
c
c
        subroutine q2leges(eps, x, nmax, vals)
        implicit double precision (a-h, o-z)
        double precision vals(0:nmax)
c
        done = 1
        blowup = 1/eps*50
c
...
c
        do i = 2,100000
          val = ((i-done)*4*x*val1 - (2*i-3)*val0)/(2*i-1)
...
c
     ...
     1      vals, grads, hesses)
        implicit double precision (a-h,o-z)
        double precision :: dum0, src(2), targ(2), dum2
        double precision :: vals(0:nmax)
        double precision :: grads(2,0:nmax), hesses(2,2,0:nmax)
c
...
c
     ...
     1      dr0s,dz0s)
        implicit double precision (a-h,o-z)
        double precision :: vals(0:1),drs(0:1),dzs(0:1)
        double precision :: dr0s(0:1),dz0s(0:1)
        double precision, allocatable :: pvals(:),qders(:)
c
c       this routine calculates the first nmax+1 laplace
c       fourier modes, i.e. the functions
c
...
c
        ...
        implicit double precision (a-h,o-z)
        double precision pvals(0:1),qvals(0:1),pders(0:1),qders(0:1)
c
...
c
        ...
        implicit double precision (a-h,o-z)
        double precision pvals(0:1),qvals(0:1)
c
...
c
        ...
        implicit double precision (a-h,o-z)
        double precision vals(0:1),ders(0:1)
c
